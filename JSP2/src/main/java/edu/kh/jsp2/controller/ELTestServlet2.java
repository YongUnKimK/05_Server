...
public class ELTestServlet2 extends HttpServlet {
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		req.setAttribute("requestValue", "request sscope 객체에 세팅한 값");
		// requestValue = request scope 객체에 세팅한 값
		
		System.out.println(req.getAttribute("requestValue"));
		//request sscope 객체에 세팅한 값
		
		// 3. session scope
		
		// - 클라이언트가 서버에 첫 요청 시 서버쪽에 생성되는 객체
		
		// - 클라이언트가 브라우저를 종료 하거나 지정된 세선 만료 시간이 지나면 사라짐
		...
		// 2) 값 세팅
		application.setAttribute("applicationValue", "application scpe 객체에 세팅한 값");
		...
		session.setAttribute("menu", "짜짱(session)");
