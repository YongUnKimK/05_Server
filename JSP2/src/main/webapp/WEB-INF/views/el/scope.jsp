...
<body>
	...
	<pre>
		...
		<h3> 1. page : 현재 페이지 </h3>
		-> 현재 JSP에서만 사용 가능한 객체 ( Servlet에서 생성 X )
		
		<h3> 2. request (요청) == HttpServletRequest</h3>
		-> 요청 받은 페이지(Servlet/JSP)와 위임받은 페이지(Servlet/JSP) 에서
		유지되는 객체
		
		<h3> 3. session (입회, 접속) </h3>
		- session : 서버에 접속한 클라이언트를 나타내거나, 관련 정보를 get/set
		 할 수 있는 객체 ( session 객체는 서버에서 관리함 )
	
		- [중요!] session은 브라우저 마다 하나씩 생성된다!!	
		
		- [유지 범위] 
		사이트 접속 ~ 브라우저 종료 | 세선 만료
		
		- session이 유지되는 상태에서는 아무곳에서나 가져다가 사용할 수 있다.
		...
		<h3> 내장 객체의 우선 순위 : page > request > session > application (유효범위가 작은 객체가 우선순위 높음)</h3>
	</pre>
