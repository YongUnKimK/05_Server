...
public class TodoListDAOImpl implements TodoListDAO {
	...
	public TodoListDAOImpl() throws Exception{
		...
		if(!file.exists()) { // 파일이 존재하지 않는다면
			...
			todoList.add(new Todo("JSP 공부", " 수업 내용 미드 오리아나~222"));
			...
			try { // catch는 throws 할것임
				ois = new ObjectInputStream(new FileInputStream(FILE_PATH));
				todoList = (ArrayList)ois.readObject();
	...
	public int todoAdd(Todo todo) throws Exception {
		...
		if(todoList.add(todo) ) {
			saveFile();// 외부파일로 출력
			return todoList.size() -1 ; //마지막 인덱스번호 반환
		}
